...
class _GoodsFilterPage extends State<GoodsFilterPage> {
  final ScrollController _leftListController =
      ScrollController(initialScrollOffset: 0);
  final ScrollController _rightListController =
      ScrollController(initialScrollOffset: 0);

  //这里给右侧List中的Grid一个初始化高度，会在_calculation（）实时计算，这里可以是任何正值
  double _rightGridItemHight = 10;
  ...
  //计算右侧的一页高度
  void _calculationGrid() {
    //这个数组用来计算单独一个List的页面高度
    List rightListStep = [];
    _rightOffSetStepList.clear();

    _rightGridItemHight = (MediaQuery.of(context).size.width - 10) *
        rightWidgetFlex /
        (leftWidgetFlex + rightWidgetFlex) /
        rightGridViewCrossAxisCount;
    rightListStep.add((_rightGridItemHight *
            ((ll[0] ~/ rightGridViewCrossAxisCount) +
                (ll[0] % rightGridViewCrossAxisCount == 0 ? 0 : 1)) +
        rightItemTitleHeight));
    for (int i = 1; i < _left.length; i++) {
      rightListStep.add((_rightGridItemHight *
              ((ll[i] ~/ rightGridViewCrossAxisCount) +
                  (ll[i] % rightGridViewCrossAxisCount == 0 ? 0 : 1)) +
          rightItemTitleHeight));
    }

    _lastGridHeight = (MediaQuery.of(context).size.height -
  ...
  void _calculation() {
    ...
    rightListStep.add(
        (_rightGridItemHight * ((ll[0]) + (ll[0] % 1 == 0 ? 0 : 1)) +
            rightItemTitleHeight));
    for (int i = 1; i < _left.length; i++) {
      rightListStep.add(
          (_rightGridItemHight * ((ll[i]) + (ll[i] % 1 == 0 ? 0 : 1)) +
              rightItemTitleHeight));
    }
      ...
      child: ListView.builder(
        ...
        itemBuilder: (context, index) {
          return GestureDetector(
            onTap: () {
              _rightListController.animateTo(_rightOffSetStepList[index],
