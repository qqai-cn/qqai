...
part 'fabu_aixin_providers.freezed.dart';
part 'fabu_aixin_providers.g.dart';

// FabuAiXinController 状态 - 使用 Freezed
@freezed
sealed class FabuAiXinState with _$FabuAiXinState {
  const factory FabuAiXinState({
  ...
  // 工厂构造函数用于初始化
  factory FabuAiXinState.initial() => FabuAiXinState(
    publishController: TextEditingController(),
    whoCanSee: const ['公开', '私密', '仅好友可看'],
  );
}

// FabuAiXinController Provider - 使用 Riverpod 3 代码生成（autoDispose）
@Riverpod(keepAlive: false)
class FabuAiXinNotifier extends _$FabuAiXinNotifier {
  @override
  FabuAiXinState build() {
    final state = FabuAiXinState.initial();
    ref.onDispose(() {
...
// Provider 已通过代码生成自动创建为 fabuAiXinNotifierProvider
