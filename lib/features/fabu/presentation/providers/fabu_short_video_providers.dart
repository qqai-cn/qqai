...
part 'fabu_short_video_providers.freezed.dart';
part 'fabu_short_video_providers.g.dart';

// FabuShortVideoController 状态 - 使用 Freezed
@freezed
sealed class FabuShortVideoState with _$FabuShortVideoState {
  const factory FabuShortVideoState({
  ...
  }) = _FabuShortVideoState;
  
  // 工厂构造函数用于初始化
  factory FabuShortVideoState.initial() => FabuShortVideoState(
    publishController: TextEditingController(),
    whoCanSee: const ['公开', '私密', '仅好友可看'],
  );
}

// FabuShortVideoController Provider - 使用 Riverpod 3 代码生成（autoDispose）
@Riverpod(keepAlive: false)
class FabuShortVideoNotifier extends _$FabuShortVideoNotifier {
  @override
  FabuShortVideoState build() {
    final state = FabuShortVideoState.initial();
    ref.onDispose(() {
...
// Provider 已通过代码生成自动创建为 fabuShortVideoNotifierProvider
